...
const getShiftStatus = (date: Date, pattern: ShiftPattern): DayStatus | null => {
...
const Calendar: React.FC<CalendarProps> = ({ currentDate, setCurrentDate, shiftPattern }) => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div key={day} className={dayCellClasses}>
          ...
          {statusText && (
            <div className={`absolute inset-0 flex items-center justify-center text-2xl sm:text-3xl md:text-4xl font-bold ${statusColor} opacity-75`}>
              {statusText}
            </div>
          )}
        </div>
  ...
  const renderLegend = () => {
    const legendItems = [
      {label: '주간', color: 'bg-yellow-100', borderColor: 'border-yellow-200'},
      {label: '야간', color: 'bg-indigo-100', borderColor: 'border-indigo-200'},
      {label: '휴무', color: 'bg-red-100', borderColor: 'border-red-200'}
    ];

    return legendItems.map(item => (
        <div key={item.label} className="flex items-center space-x-2">
          <div className={`w-4 h-4 rounded-full ${item.color} border ${item.borderColor}`}></div>
